<x-app-layout>
    <form method="POST" action="{{ route('customers.storeInvitation') }}">
        @csrf
        <x-slot name="header">
            <div class="py-4 px-4 bg-white shadow sm:rounded-lg flex items-center justify-between">
                <h2 class=" text-xl text-gray-500 leading-tight">
                    <i class="fa-solid fas fa-envelope-open-text mr-2 text-[#B6844A]"></i>{{ __('Invite Customer') }}
                </h2>

            </div>
        </x-slot>

        <div class="bg-white shadow sm:rounded-lg p-6 ml-3 mr-3 mt-2">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4">
                <div class="flex items-center gap-2 mt-3">
                    ...
                    <input type="checkbox" class="h-4 w-4" name="is_luxury_magazine_lead" value="1">
        ...
        <div id="inviteCustomerPage" data-email-exists="{{ session('email_exists') ? '1' : '0' }}"></div>
    </form> 
</x-app-layout>
<x-invite-customer-attention-modal />