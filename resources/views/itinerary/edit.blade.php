@php
    ...
    $signedUrl = \Illuminate\Support\Facades\URL::temporarySignedRoute('itinerary.view',now()->addDays(30),['itinerary' => $itinerary->id]);
@endphp
                ...
                @if(!$viewOnly)
                    <div class="col-span-4 flex flex-col gap-1 items-end">
                        ...
                        <div class="flex gap-2">
                            <x-primary-btn id="editBtn" class="flex gap-2 hidden"><i class="fas fa-edit mt-1"></i>Edit</x-primary-btn>
                            <x-primary-btn id="doneBtn" class="flex gap-2 hidden px-19">Done</x-primary-btn>
                            <x-primary-btn id="addEventBtn" onclick="openManageEventItineraryModal()" class="flex gap-2 hidden"><i class="far fa-plus-square mt-1"></i>Add Event</x-primary-btn>
                        </div>

                        <div class="flex gap-2 mt-2 w-full">
                            <button class="flex-1 text-white px-4 xl:px-8 py-2 rounded text-sm transition cursor-pointer" :class="section === 'yourItems' ? 'bg-[#B6844A]': 'bg-[#696969]'" @click="section = 'yourItems'">
                                Your Items
                            </button>

                            <button  class="flex-1 text-white px-4 xl:px-8 py-2 rounded text-sm transition cursor-pointer" :class="section === 'guides' ? 'bg-[#B6844A]' : 'bg-[#696969]'" @click="section = 'guides'">
                                Guides
                            </button>
                        </div>
                            ...
                            <div class="flex flex-col" x-show="section === 'guides'" x-cloak>
                                ...
                                <div x-data="{ guideSearchSection: 'searchByCity' }" class="flex flex-col gap-2">
                                    <div class="flex gap-2 mt-2">
                                        <button class="text-white px-3 py-2 rounded text-sm transition cursor-pointer" :class="guideSearchSection === 'searchByCity' ? 'bg-[#B6844A]' : 'bg-[#696969]'" @click="guideSearchSection = 'searchByCity'">
                                            Search By City
                                        </button>

                                        <button class="text-white px-3 py-2 rounded text-sm transition cursor-pointer" :class="guideSearchSection === 'searchByCountry' ? 'bg-[#B6844A]' : 'bg-[#696969]'" @click="guideSearchSection = 'searchByCountry'">
                                            Search By Country
                                        </button>
                                    </div>
