<x-app-layout>
    <x-slot name="header">
        <div class="py-4 px-4 bg-white shadow sm:rounded-lg flex items-center justify-between">
            <h2 class=" text-xl text-gray-500 leading-tight">
                <i class="fa-solid fa-flag-usa mr-2 text-[#B6844A]"></i>{{ __('Booked Trips by State') }}
            </h2>
            
            <button class="flex items-center gap-2 bg-white border py-2 px-4 border-[#B6844A] text-[#B6844A] cursor-pointer"><i class="fas fa-cloud-download-alt"></i>Download</button>

            <button class="flex items-center gap-2 items-center" onclick="openReportDateRangeModal()">
                <i class="far fa-calendar-alt text-[#212121] text-2xl"></i>
                <span id="selectedReportDates" class="text-[#212121] text-base font-bold"></span>
            </button>
        </div>
    </x-slot>

    <div class="p-2">  
       
    </div>

</x-app-layout>
<x-report-range-date>
    <x-slot name="footer">
        <x-primary-btn id="bookedTripsByStateReportBtn"><i class="fa fa-paper-plane"></i><span>Run Report</span></x-primary-btn>
        <x-secondary-btn onclick="closeReportDateRangeModal()"><i class="fa fa-times-circle"></i><span>Cancel</span></x-secondary-btn>
    </x-slot>
</x-report-range-date>
