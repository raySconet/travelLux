<form method="POST" enctype="multipart/form-data" action="{{ $isNewUser ? route('system-users.store') : route('system-users.update', $user->id ) }}">
    @csrf
    @if(!$isNewUser)
        @method('PUT')
    @endif    
    <x-app-layout>
        <x-slot name="header">
            <div class="py-4 px-4 bg-white shadow sm:rounded-lg flex items-center justify-between">
                ...
                @else    
                    <div class="space-x-2">
                        <x-secondary-buttonToDelete type="button" onclick="openDeleteModal(document.getElementById('deleteUserForm'))">
...
@if(!$isNewUser)
    <form method="POST" action="{{ route('system-users.destroy', $user->id) }}" id="deleteUserForm">
        @csrf
        @method('DELETE')
    </form>
@endif
